...
public class CreatePageController {
    ...
    public CreatePageController(CharacterGenerationService characterGenerationService) {
    ...
    public String submitQuestionnaire(
        ...
        //c фронта мы берем ответ_и айдишка, а это чтобы его на
        try {
            Map<Long, Long> selectedAnswers = new HashMap<>();
            for (Map.Entry<String, String> entry : params.entrySet()) {//чтобы получить одним списком ключ-пары
                String key = entry.getKey();
                if (!key.startsWith("answer_")) {
                    continue;
                }
                long questionId = Long.parseLong(key.substring("answer_".length()));
                //берет только айдишку
                //переводит его в лонг
                long answerId = Long.parseLong(entry.getValue());
                //пэйрит айдишку вопроса и ответа
